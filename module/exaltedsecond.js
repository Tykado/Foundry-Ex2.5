...
import { exaltedsecondActorSheet } from "./actor/actor-sheet.js";
import { exaltedsecondActor } from "./actor/actor.js";
import * as Chat from "./chat.js";
import { exaltedsecondItemSheet } from "./item/item-sheet.js";
import { exaltedsecondItem } from "./item/item.js";
...
Hooks.once('init', async function () {
  ...
  CONFIG.Combat.initiativeIcon = {
    icon: "../systems/exaltedsecond/assets/icons/d10.svg",
    hover: "../systems/exaltedsecond/assets/icons/d10.svg"
  };
  ...
  CONFIG.exaltedsecond = exaltedsecond;
  CONFIG.statusEffects = exaltedsecond.statusEffects;
  ...
  foundry.applications.apps.DocumentSheetConfig.registerSheet(ActiveEffect, "exaltedsecond", ExaltedActiveEffectConfig, { makeDefault: true });

  CONFIG.ActiveEffect.sheetClass = ExaltedActiveEffectConfig;
  CONFIG.ActiveEffect.legacyTransferral = false;

  game.socket.on('system.exaltedsecond', handleSocket);

  // Register sheet application classes
  foundry.documents.collections.Actors.unregisterSheet("core", foundry.appv1.sheets.ActorSheet);
  foundry.documents.collections.Actors.registerSheet("exaltedsecond", exaltedsecondActorSheet, { makeDefault: true });
  foundry.documents.collections.Items.unregisterSheet("core", foundry.appv1.sheets.ItemSheet);
  foundry.documents.collections.Items.registerSheet("exaltedsecond", exaltedsecondItemSheet, { makeDefault: true });

  // Pre-load templates
  foundry.applications.handlebars.loadTemplates([
    "systems/exaltedsecond/templates/dialogues/added-charm-list.html",
    "systems/exaltedsecond/templates/actor/active-effects.html",
    "systems/exaltedsecond/templates/item/custom-modifier.html",
    "systems/exaltedsecond/templates/item/item-trigger-list.html",
    "systems/exaltedsecond/templates/actor/equipment-list.html",
    "systems/exaltedsecond/templates/actor/npc-stats.html",
    "systems/exaltedsecond/templates/actor/character-stats.html",
  ]);
...
Handlebars.registerHelper('isBooleanTrigger', function (bonusEffect, options) {
  return CONFIG.exaltedsecond.booleanTriggers.includes(bonusEffect) ? options.fn(this) : options.inverse(this);
});
...
Hooks.on("renderActorDirectory", (app, html, data) => {
  ...
  html.querySelectorAll('.character-generator-button').forEach(element => {
    element.addEventListener('click', async (ev) => {
      new CharacterBuilder({ classes: [" exaltedsecond exaltedsecond-dialog", `${game.settings.get("exaltedsecond", "sheetStyle")}-background`] }, {}).render(true);
    });
  });
  ...
  html.querySelectorAll('.resume-character')
    .forEach((target) => {
      target.addEventListener('click', async (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        new CharacterBuilder({ classes: ["exaltedsecond exaltedsecond-dialog", `${game.settings.get("exaltedsecond", "sheetStyle")}-background`] }, message.flags?.exaltedsecond?.character).render(true);
      });
    });
  html.querySelectorAll('.add-oppose-charms')
    .forEach((target) => {
      target.addEventListener('click', async (ev) => {
        ...
        if (message.flags?.exaltedsecond?.targetActorId) {
          actor = canvas.tokens.placeables.find(t => t.id === message.flags?.exaltedsecond?.targetTokenId)?.actor;
          if (!actor) {
            actor = game.actors.get(message.flags?.exaltedsecond?.targetActorId);
          }
        ...
        if (message.flags?.exaltedsecond?.rollerUuid) {
          attacker = await fromUuid(message.flags.exaltedsecond.rollerUuid);
        }
  ...
  html.querySelectorAll('.make-chat-roll')
    .forEach((target) => {
      target.addEventListener('click', async (ev) => {
        ev.preventDefault();
        ev.stopPropagation();
        const attribute = message.flags?.exaltedsecond?.attribute;
        const ability = message.flags?.exaltedsecond?.ability;
        const difficulty = message.flags?.exaltedsecond?.difficulty;
...
Hooks.on('updateCombat', (async (combat, update, diff, userId) => {
  ...
  if (update && update.round && update.round !== 1) {
    for (var combatant of combat.combatants) {
      ...
      for (const activeEffect of combatant.actor.allApplicableEffects()) {
        if (activeEffect.duration.remaining >= 0 && !activeEffect.disabled) {
          for (const change of activeEffect.changes) {
            ...
            if (CONFIG.exaltedsecond.diceRollActiveEffects.includes(change.key)) {
            ...
            if (change.key === 'system.damage.round.initiative.lethal' || change.key === 'system.damage.round.initiative.bashing') {
              if (currentCombatantInitiative !== null && (currentCombatantInitiative - rolledChangeValue) <= 0 && currentCombatantInitiative > 0) {
                crasherId = activeEffect.flags?.exaltedsecond?.poisonerCombatantId;
              }
        ...
        if (activeEffect.flags?.exaltedsecond?.weaponInflictedPosion && activeEffect.duration.remaining === 0) {
      ...
      if (moteCost && !game.settings.get("exaltedsecond", "gloryOverwhelming")) {
      ...
      if (combatant.flags?.crashRecovery) {
        await combatant.update({
          [`flags.exaltedsecond.crashRecovery`]: (combatant.flags?.exaltedsecond?.crashRecovery ?? 2) - 1,
        });
      }
  ...
  if (update && (update.round !== undefined || update.turn !== undefined)) {
    ...
    if (combat.current.combatantId) {
      ...
      if (currentCombatant?.actor) {
        const onslaught = currentCombatant.actor.effects.find(i => i.flags.exaltedsecond?.statusId == "onslaught");
        if (onslaught) {
        ...
        const defensePenalty = currentCombatant.actor.effects.find(i => i.flags.exaltedsecond?.statusId == "defensePenalty");
        if (defensePenalty) {
...
Hooks.on("deleteCombat", (entity, deleted) => {
  for (const combatant of entity.combatants) {
    if (combatant?.actor) {
      const onslaught = combatant.actor.effects.find(i => i.flags.exaltedsecond?.statusId == "onslaught");
      if (onslaught) {
...
Hooks.on("renderPause", function () {
  const iconSrc = game.settings.get("exaltedsecond", "pauseIcon");
  document.querySelectorAll(".paused img").forEach(img => {
    img.src = `systems/exaltedsecond/assets/pause/${iconSrc}.png`;
  });
});
...
Hooks.once("ready", async function () {
  ...
  const gameMigrationVersion = game.settings.get("exaltedsecond", "systemMigrationVersion");

  if (foundry.utils.isNewerVersion("1.4.1", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
    for (let item of game.items) {
      ...
      await game.settings.set("exaltedsecond", "systemMigrationVersion", game.system.version);
    }
  }

  if (foundry.utils.isNewerVersion("1.4.3", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("1.7.6", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("1.9.2", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
    for (let item of game.items) {
      try {
        if (item.type === 'charm') {
          let updateData = foundry.utils.deepClone(item.toObject());
  ...
  if (foundry.utils.isNewerVersion("1.9.5", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
    for (let actor of game.actors.filter((actor) => actor.type === 'npc')) {
      try {
        let updateData = foundry.utils.duplicate(actor);
  ...
  if (foundry.utils.isNewerVersion("1.10.0", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
    for (let actor of game.actors.filter((actor) => actor.type === 'npc')) {
      try {
        let updateData = foundry.utils.duplicate(actor);
        if (updateData.system.creaturetype !== 'exalt') {
  ...
  if (foundry.utils.isNewerVersion("1.11.0", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.0.3", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.4.0", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.5.2", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.7.0", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.7.4", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("2.7.5", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  if (foundry.utils.isNewerVersion("3.7.8", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
  ...
  // if (foundry.utils.isNewerVersion("2.8.0", game.settings.get("exaltedsecond", "systemMigrationVersion"))) {
...
async function createItemMacro(data, slot) {
  ...
  if (data.type === "Item") {
    ...
    const command = `const formActor = await fromUuid("${data.actorId}");
        game.rollForm = new game.exaltedsecond.RollForm(${data.actorId.includes('Token') ? 'formActor.actor' : 'formActor'}, {classes: [" exaltedsecond exaltedsecond-dialog", "${game.settings.get("exaltedsecond", "sheetStyle")}-background"]}, {}, { rollId: "${data.id}" }).render(true); `;
    const macro = await Macro.create({
      name: data.name,
      img: 'systems/exaltedsecond/assets/icons/d10.svg',
      type: "script",
      command: command,
    });
    game.user.assignHotbarMacro(macro, slot);
...
function applyDecisiveDamage(message) {
  ...
  if (!targetedTokens?.length || !game.settings.get("exaltedsecond", "rollButtonTarget")) {
    const targetToken = game.canvas.tokens.get(message?.flags?.exaltedsecond?.targetId);
    if (targetToken) {
  ...
  const damageContext = {
    value: message?.flags?.exaltedsecond?.damage?.total || 0,
    type: message?.flags?.exaltedsecond?.damage?.type || 'lethal',
    attackerTokenId: message?.flags?.exaltedsecond?.attackerTokenId,
  };
...
function applyWitheringDamage(message) {
  ...
  for (const token of targetedTokens) {
    ...
    if (combatant && combatant.initiative !== null) {
      var initiativeDamage = message?.flags?.exaltedsecond?.damage?.total;
      if (actor.type !== 'npc' || actor.system.battlegroup === false) {
        if (game.settings.get("exaltedsecond", "useShieldInitiative") && actor.system.newShieldInitiative.value > 0) {
          var newShieldInitiative = Math.max(0, actor.system.newShieldInitiative.value - message?.flags?.exaltedsecond?.damage?.total);
          initiativeDamage = Math.max(0, message?.flags?.exaltedsecond?.damage?.total - actor.system.newShieldInitiative.value);
          this.actor.update({ [`system.newShieldInitiative.value`]: newShieldInitiative });
        }
        game.combat.setInitiative(combatant.id, combatant.initiative - initiativeDamage, message?.flags?.exaltedsecond?.attackerCombatantId);
      }
    }
    if (actor.system?.battlegroup) {
      dealHealthDamage(actor, message?.flags?.exaltedsecond?.damage?.total, message?.flags?.exaltedsecond?.damage?.type || 'lethal');
    }
  }
}
...
function gainAttackInitiative(message) {
  ...
  // If there are not any controlled tokens, issue a warning.
  if (!targetedTokens?.length || !game.settings.get("exaltedsecond", "rollButtonTarget")) {
    const targetToken = game.canvas.tokens.get(message?.flags?.exaltedsecond?.attackerTokenId);
    if (targetToken) {
      targetedTokens = [targetToken];
    }
    else {
  ...
  for (const token of targetedTokens) {
    ...
    if (combatant && combatant.initiative !== null) {
      game.combat.setInitiative(combatant.id, combatant.initiative + message?.flags?.exaltedsecond?.damage?.gainedInitiative);
    }
  }
}
...
async function applyDamageDialogue(targetUuid, damageContext) {
  ...
  const template = "systems/exaltedsecond/templates/dialogues/damage-dialogue.html"
  const html = await foundry.applications.handlebars.renderTemplate(template, { 'damageContext': damageContext, damageTypes: CONFIG.exaltedsecond.selects.damageTypes });
  new foundry.applications.api.DialogV2({
...
function weaponAttack(itemUuid, attackType = 'withering') {
  ...
  Item.fromDropData(dropData).then(item => {
    ...
    game.rollForm = new RollForm(item.parent, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", `${game.settings.get("exaltedsecond", "sheetStyle")}-background`] }, {}, { rollType: attackType, weapon: item.system }).render(true);
  });
}
...
/**
 * 
 * @param {exaltedsecondActor} actor 
 * @param {object} object 
 * @param {object} data 
 * @returns {Promise}
 */
function roll(actor, object, data) {
  return new RollForm(actor, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller"] }, {}, data).roll();
}