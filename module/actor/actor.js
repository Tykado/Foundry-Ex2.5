...
export class exaltedsecondActor extends Actor {
  ...
  async _preUpdate(updateData, options, user) {
    ...
    if (caste !== this.system.details.caste && this.type === 'character') {
      ...
      for (let [key, ability] of Object.entries(abilities)) {
        ...
        if ((this.items.some(charm => charm.type === 'charm' && charm.system.ability === key) || (ability.favored && abilityValue > 0)) && CONFIG.exaltedsecond.abilityExalts.includes(caste)) {
      ...
      if (this.system.details.exalt === 'exigent') {
        if (updateData.system?.settings) {
          updateData.system.settings.exigenttype = CONFIG.exaltedsecond.exigentTiers[caste];
        }
        else {
          updateData.system.settings = {
            exigenttype: CONFIG.exaltedsecond.exigentTiers[caste]
          }
        }
        updateData.system.traits = {
  ...
  canAquireItem(item) {
    ...
    if (itemType === 'charm') {
      ...
      if (item.system.numberprerequisites.number > 0) {
        ...
        else if (CONFIG.exaltedsecond.maidens.includes(item.system.ability)) {
      ...
      if (CONFIG.exaltedsecond.maidens.includes(item.system.ability) && item.system.requirement > this._getHighestMaidenAbility(item.system.ability)) {
        validAbilityRequirement = false;
      }
      ...
      if (item.system.archetype.ability || item.system.archetype.charmprerequisites.length > 0) {
        if (item.system.archetype.ability) {
          ...
          if (CONFIG.exaltedsecond.maidens.includes(item.system.archetype.ability) && item.system.requirement > this._getHighestMaidenAbility(item.system.archetype.ability)) {
            return false;
          }
        }
  ...
  _getMaidenCharmsNumber(maiden) {
    return (this.items.filter(numberCharm => numberCharm.type === 'charm' && CONFIG.exaltedsecond.maidenabilities[maiden].includes(numberCharm.system.ability)).length || 0)
  }
...
calculateMaxExaltedMotes(moteType, exaltType, essenceLevel) {
  ...
  if (moteType === 'personal') {
    const pools = CONFIG.exaltedsecond?.exaltMotePools?.personal ?? {};
  ...
  // PERIPHERAL MOTES (Virtues included)
    ...
    const pools = CONFIG.exaltedsecond?.exaltMotePools?.peripheral ?? {};
    const base = Number(pools.base?.[exaltType] ?? pools.base?.default ?? 0);
    const mult = Number(pools.essenceLevelMultiplier?.[exaltType] ?? pools.essenceLevelMultiplier?.default ?? 0);
  ...
  async savedRoll(name) {
    ...
    if (data.rollType === 'useOpposingCharms') {
      game.opposingCharmForm = await new RollForm(this, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", this.getSheetBackground()] }, {}, { rollId: roll.id, skipDialog: true }).roll();
    } else {
      game.rollForm = await new RollForm(this, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", this.getSheetBackground()] }, {}, { rollId: roll.id, skipDialog: true }).roll();
    }
  }
  ...
  getSavedRoll(name) {
    ...
    return new RollForm(this, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", this.getSheetBackground()] }, {}, { rollId: roll.id });
  }
  /**
    ...
    for (let i of actorCharms) {
      ...
      if (i.system.listingname) {
        ...
        if (charms[i.system.ability]) {
          ...
          charms[i.system.ability] = { name: CONFIG.exaltedsecond.charmabilities[i.system.ability] || 'Ex3.Other', visible: true, list: [], collapse: this.charms ? this.charms[i.system.ability]?.collapse : true };
        }
...
return 0;
  ...
  async actionRoll(data) {
    ...
    if (data.rollType === 'useOpposingCharms') {
      game.opposingCharmForm = await new RollForm(this, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", this.getSheetBackground()], position: { width: 846, height: 642 } }, {}, data).render(true);
    } else {
      game.rollForm = await new RollForm(this, { classes: [" exaltedsecond exaltedsecond-dialog dice-roller", this.getSheetBackground()], position: { width: 763 } }, {}, data).render(true);
    }
  }

  getSheetBackground() {
    if (this.system.settings.sheetbackground === 'default') {
      return `${game.settings.get("exaltedsecond", "sheetStyle")}-background`;
    }
    return `${this.system.settings.sheetbackground}-background`;
  }

  async sendTargetingChatMessage(data) {
    const imageUrl = CONFIG.exaltedsecond.rollTypeTargetImages[data.attackType] || CONFIG.exaltedsecond.rollTypeTargetImages[data.rollType] || CONFIG.exaltedsecond.rollTypeTargetImages[data.ability] || "systems/exaltedsecond/assets/icons/d10.svg";
    const rollTypeLabel = CONFIG.exaltedsecond.rollTypeTargetLabels[data.attackType] || CONFIG.exaltedsecond.rollTypeTargetLabels[data.rollType] || CONFIG.exaltedsecond.rollTypeTargetLabels[data.ability] || "Ex3.Roll";
    if (game.user.targets && game.user.targets.size > 0) {
      for (const target of Array.from(game.user.targets)) {
        const messageContent = await foundry.applications.handlebars.renderTemplate("systems/exaltedsecond/templates/chat/targeting-card.html", {
          actor: this,
    ...
    } else if (CONFIG.exaltedsecond.targetableRollTypes.includes(data.rollType)) {
      const messageContent = await foundry.applications.handlebars.renderTemplate("systems/exaltedsecond/templates/chat/targeting-card.html", {
        actor: this,
        targetActor: null,
        imgUrl: imageUrl,
    ...
    } else if (game.settings.get("exaltedsecond", "nonTargetRollCards")) {
      const messageContent = await foundry.applications.handlebars.renderTemplate("systems/exaltedsecond/templates/chat/pre-roll-card.html", {
        actor: this,
        imgUrl: imageUrl,
        rollType: rollTypeLabel,
      });
  ...
  alterDefensePenalty(direction = "increase", statusId = "defensePenalty") {
    let icon = 'systems/exaltedsecond/assets/icons/slashed-shield.svg';
    let label = "Defense Penalty";
    if (statusId === 'onslaught') {
      label = "Onslaught";
      icon = 'systems/exaltedsecond/assets/icons/surrounded-shield.svg';
    }
    const existingPenalty = this.effects.find(i => i.flags.exaltedsecond?.statusId === statusId);
