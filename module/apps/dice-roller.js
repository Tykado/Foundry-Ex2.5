...
export default class RollForm extends HandlebarsApplicationMixin(ApplicationV2) {
    constructor(actor, options, object, data) {
        ...
        if (data.lastRoll) {
            ...
            this.object.opposingDiceRoll = null;
            this.object.opposingDiceDisplay = null;
            this.object.opposingDiceTotal = 0;
            ...
            this.object.roll = null;
            this.object.diceDisplay = null;
            this.object.diceRollTotal = null;
        ...
        if (this.object.rollType !== "base") {
            for (let [key, target] of Object.entries(this.object.targets)) {
                if (target.rollData.defenseType !== formObject.object.targets[key].rollData.defenseType) {
                    ...
                    } else if (formObject.object.targets[key].rollData.defenseType === 'clash') {
                        if (target.rollData.opposingRollTotal) {
                            formData.object[`object.targets.${key}.rollData.defense`] = target.rollData.opposingRollTotal;
                        } else {
                            formData.object[`object.targets.${key}.rollData.defense`] = 0;
                        }
                    } else {
    ...
    _setUpMultitargets() {
        ...
        for (const target of Array.from(game.user.targets)) {
            target.rollData = {
                ...
                soak: 0,
                difficulty: 0,
                opposingDiceRoll: null,
                opposingDiceDisplay: null,
                opposingRollTotal: null,
                shieldInitiative: 0,
                diceModifier: 0,
                successModifier: 0,
                damageModifier: 0,
    ...
    _enableSpecialAttack(id) {
        ...
        if (id === 'aim') {
            ...
            if (id === 'knockback' || id === 'knockdown') {
                ...
                for (const target of Object.values(this.object.targets)) {
                    target.rollData.defenseType = 'clash';
                    if (target.rollData.opposingRollTotal) {
                        target.rollData.defense = target.rollData.opposingRollTotal;
                    } else {
                        target.rollData.defense = 0;
                    }
                }
    ...
    _getSpecificDiceTriggerFormula(formula, overrideActor = null, diceRollType) {
        ...
        if (diceRollType === 'opposed') {
            if (this.object.opposingDiceRoll) {
                this._specificDieFaceEffect(this.object.opposingDiceRoll, newEffect);
            } else {
                this.object.effectsOnSpecificDice.push(newEffect);
            }
        }
    ...
    async addOpposingRoll(data) {
        if (this.object.showTargets) {
            for (const target of Object.values(this.object.targets)) {
                if (target.actor.id === data.actor._id || this.object.showTargets === 1) {
                    target.rollData.opposingDiceRoll = data.diceRoll;
                    target.rollData.opposingDiceDisplay = data.diceDisplay;
                    target.rollData.opposingRollTotal = data.total;
                    if (target.rollData.difficulty === 0) {
                        target.rollData.difficulty = data.total;
                    }
                    if (target.rollData.defenseType === 'clash') {
                        target.rollData.defense = data.total;
                    }
                }
            }
        } else {
            this.object.opposingDiceRoll = data.diceRoll;
            this.object.opposingDiceDisplay = data.diceDisplay;
            if (this.object.difficulty === 0) {
                this.object.difficulty = data.total;
                this.object.opposingDiceTotal = data.total;
            }
        }
        this.render();
    }
    ...
    async _baseOrAbilityDieRoll() {
        ...
        if (onesRolled > 0 && this.object.settings.triggerOnOnes !== 'none') {
            switch (this.object.settings.triggerOnOnes) {
                ...
                case 'subtractSuccesses':
                    this.object.diceRollTotal -= onesRolled;
                    break;
                case 'reduceDamage':
        ...
        if (tensRolled > 0 && this.object.settings.triggerOnTens !== 'none') {
            switch (this.object.settings.triggerOnTens) {
                ...
                case 'postSoakDamage':
                    ...
                    break;
                case 'extraSuccess':
                    this.object.diceRollTotal += tensRolled;
                    break;
                case 'ignoreHardness':
        ...
        if (this.object.opposingDiceRoll) {
            for (const dieFaceTrigger of this.object.effectsOnSpecificDice.filter(faceTrigger => faceTrigger.diceRollType === 'opposed')) {
                this._specificDieFaceEffect(this.object.opposingDiceRoll, dieFaceTrigger);
            }
        }
        if (this.object.doubleSuccesses) {
            this.object.diceRollTotal *= 2;
        }
        if (this._isAttackRoll()) {
            this.object.thresholdSuccesses = Math.max(0, this.object.diceRollTotal - this.object.defense);
            if (this.object.doubleThresholdSuccesses) {
                this.object.thresholdSuccesses *= 2;
            }
            this.object.attackSuccesses = this.object.diceRollTotal;
            if (this.object.target) {
                this.object.target.rollData.attackSuccesses = this.object.diceRollTotal;
                if (this.object.rollAccuracyOnce) {
                    for (const target of Object.values(this.object.targets)) {
                        target.rollData.attackSuccesses = this.object.diceRollTotal;
                    }
                }
            }
        }
        else {
            this.object.thresholdSuccesses = Math.max(0, this.object.diceRollTotal - (this.object.difficulty || 0));
            if (this.object.doubleThresholdSuccesses) {
                this.object.thresholdSuccesses *= 2;
            }
        }
    ...
    async _baseRoll() {
        ...
        <div class="dice-roll">
            <div class="dice-result">
                <h4 class="dice-total">${this.object.dice} Dice + ${this.object.successes} ${this.object.successes === 1 ? `Success` : 'Successes'}
                </h4>
    ...
    async _abilityRoll() {
        ...
        if (this.object.rollType === "sorcery") {
            if (this.object.spell) {
                ...
                const fullSpell = this.actor.items.get(this.object.spell);
                if (fullSpell) {
                    resultString += `<h4 class="dice-total">Spell Motes: ${this.object.previousSorceryMotes + this.object.diceRollTotal}/${parseInt(fullSpell.system.cost) + (crashed ? 3 : 0)}</h4>`;
                }
        ...
        if (this.object.hasDifficulty) {
            if (this.object.diceRollTotal < this.object.difficulty) {
                ...
                if (this.object.diceRollTotal === 0 && this.object.roll.dice[0].results.some((die) => die.result === 1)) {
    ...
    async _damageRoll() {
        ...
        let damageRollResults = await this._calculateRoll(dice, rollModifiers);
        if (this.object.damage.rollTwice || this.object.damage.rollTwiceLowest) {
            if (this.object.damage.rollTwice && !this.object.damage.rollTwiceLowest) {
                const secondRoll = await this._calculateRoll(dice, rollModifiers);
                if (secondRoll.total > damageRollResults.total) {
                    this.object.damageDiceRollResults = secondRoll;
                    this.object.unusedDamageDiceRollResults = damageRollResults;
                }
                else {
                    this.object.damageDiceRollResults = damageRollResults;
                    this.object.unusedDamageDiceRollResults = secondRoll;
                }
        ...
        let damageDisplayResults = `
                                ...
                                <h4 class="dice-total">${this.object.attackSuccesses} Accuracy successes vs ${this.object.defense} Defense</h4>
                                <h4 class="dice-total">${this.object.baseDamage || 0} Dice + ${this.object.damage.damageSuccessModifier} Successes</h4>
                                ${this.object.attackType === 'decisive' ? `<h4 class="dice-total">${this.object.damageDice} Damage dice vs ${this.object.hardness} Hardness ${this.object.damage.ignoreHardness ? `(Ignoring ${this.object.damage.ignoreHardness})` : ''}</h4>` : ''}
                                ${this.object.attackType === 'withering' ? `<h4 class="dice-total">${this.object.soak} Soak! ${this.object.damage.ignoreSoak ? `(Ignoring ${this.object.damage.ignoreSoak})` : ''}</h4><h4 class="dice-total">${this.object.overwhelming} Overwhelming!</h4>` : ''}
                                <div class="dice-tooltip">
        ...
        if (this.object.damage.attackDealsDamage) {
            if (this.object.attackType === 'decisive') {
                ...
                if (this._useLegendarySize('decisive')) {
                    typeSpecificResults += `<h4 class="dice-total">Legendary Size</h4><h4 class="dice-total">Damage capped at ${3 + this.actor.system.attributes.strength.value + this.object.damage.damageSuccessModifier} + Charm damage levels</h4>`;
                ...
                typeSpecificResults = `<h4 class="dice-total">${this.object.damageSuccesses} Successes vs ${this.object.gambitDifficulty} Difficulty!</h4>${resultsText}`;
    ...
    async _failedDecisive(dice) {
        ...
        if (this.object.rollType !== 'damage') {
            accuracyContent = `
                ...
                <h4 class="dice-total">${this.object.diceRollTotal} Successes vs ${this.object.defense} Defense</h4>
        ...
        <div class="dice-roll">
            <div class="dice-result">
                ${accuracyContent}
                <h4 class="dice-total">${dice} Damage vs ${this.object.hardness} Hardness ${this.object.damage.ignoreHardness ? `(Ignoring ${this.object.damage.ignoreHardness})` : ''}</h4>
                            ...
                            if ((type === 'itemAdded' || !this.object.bonusesTriggered[type] || ['defense', 'soak', 'hardness', 'guile', 'resolve'].includes(bonus.effect))) {
                                ...
                                switch (bonus.effect) {
                                    ...
                                    case 'attackClash':
                                        ...
                                        for (let specialAttack of this.object.specialAttacksList) {
                                            if (specialAttack.id === 'clash') {
                                                specialAttack.added = (typeof cleanedValue === "boolean" ? cleanedValue : true);
                                            }
                                        }
                                    ...
                                    case 'specificDieFaceEffect':
                                        ...
                                        break;
                                    case 'opposedDieFaceEffect':
                                        this._getSpecificDiceTriggerFormula(cleanedValue, triggerActor, 'opposed');
                                        break;
                                    case 'damageDice':
    ...
    async _triggerRequirementsMet(charm, trigger, bonusType = "benefit", triggerAmountIndex, display, triggerHasBeenActivatedOnItem, triggerActor) {
        ...
        for (const requirementObject of Object.values(trigger.requirements)) {
            ...
            switch (requirementObject.requirement) {
                ...
                case 'rollSucceeded':
                    if (this._isAttackRoll()) {
                        if (this.object.diceRollTotal < this.object.defense && cleanedValue === true) {
                            fufillsRequirements = false;
                        } else if (this.object.diceRollTotal >= this.object.defense && cleanedValue === false) {
                            fufillsRequirements = false;
                        }
                    } else {
                        if (this.object.diceRollTotal < this.object.difficulty && cleanedValue === true) {
                            fufillsRequirements = false;
    ...
    async _completeCraftProject() {
        ...
        let goalNumberLeft = this.object.goalNumber;
        let extendedTest = ``;
        const thresholdSuccesses = Math.max(0, this.object.diceRollTotal - this.object.difficulty);
        if (this.object.goalNumber > 0) {
    ...
    _getDiceRollDisplay(type = 'roll') {
        let rollDisplay = `
            ...
            ${type === 'roll' && this.object.unusedDiceRollDisplay ? `<div class="flex-center resource-label">Unused Roll</div><div class="dice">
    ...
    async _updateRollerResources() {
        ...
        if (this.object.rollType === 'sorcery') {
            ...
            actorSorceryMotes += this.object.diceRollTotal;
            if (this.object.spell) {
